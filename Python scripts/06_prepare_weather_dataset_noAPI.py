...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--idx", type=int, required=True,
                   help="Index of the interval file: data/processed/Intervals/[idx]_flight_data.csv")
    return p.parse_args()
...
def fetch_openmeteo(params: dict) -> dict:
    ...
    r = requests.get(url, params={**params, "apikey": "[API_KEY_HERE]"})
    r.raise_for_status()
    return r.json()
...
def main():
    args = parse_args()
    csv_path = f"data/processed/Intervals/{args.idx}_flight_data.csv"
...
def get_weather_for_idx(idx: int) -> dict:
    ...
    """
    csv_path = f"data/processed/Intervals/{idx}_flight_data.csv"
