...
# -*- coding: utf-8 -*-
"""
02A_extract_interval_points_submission.py

Submission variant of 02_extract_interval_points.py.

Differences vs. training script:
- Intervals source: data/processed/submission_intervals.csv
- Trajectories source: prc-2025-datasets/flights_rank/{flight_id}.parquet
- Output folder: data/processed/Intervals_submission

Extraction logic:
- Standard path: use real samples inside [start, end] if at least one
  "complete" row exists (lat, lon, alt, groundspeed, track, vertical_rate present).
- Fallback (inside flight span, but no complete interior rows): pick nearest valid
  anchors around the window and synthesize exactly 3 rows (start/mid/end) using:
    lat/lon/alt -> linear; track -> circular mean (mid);
    groundspeed/vertical_rate -> arithmetic mean (mid).
- Outside flight span: synthesize 3 rows at start/mid/end from nearest valid row;
  set alt=0, groundspeed=0, vertical_rate=0.
"""
...
def process_interval(row, flight_df_cache, args, outdir: Path):
    ...
    if flight_id not in flight_df_cache:
        parquet_path = Path(args.root) / "flights_final" / f"{flight_id}.parquet"
        if not parquet_path.exists():
            return f"missing_parquet:{flight_id}"
        try:
            fdf = pd.read_parquet(parquet_path, engine="pyarrow")
        except Exception:
            # Fallback to default engine if pyarrow isn't available
    ...
    if overlaps:
        ...
        if f_valid.empty:
            # No fully-valid rows — fall back to nearest-anything approach (treat like outside)
            mode = "extrapolation"
...
def main():
    ...
    ap.add_argument("--root", default="prc-2025-datasets",
                    help="Dataset root containing flights_final/")
    ap.add_argument("--features", default="data/processed/final_submission_intervals.csv",
                    help="CSV with idx,flight_id,start,end,... (submission set)")
    ap.add_argument("--outdir", default="data/processed/Intervals_submission_final",
                    help="Directory to write [idx]_flight_data.csv (submission)")
    ap.add_argument("--limit", type=int, default=None, help="Process at most N intervals")
    ap.add_argument("--preview", action="store_true", help="Dry-run without writing files")
    ...
    for _, r in tqdm(feats.iterrows(), total=len(feats), desc="Intervals (submission)"):
        try:
            if args.preview:
                # Dry run: simulate processing while forcing overwrite=True for the inner call
                status = process_interval(r, flight_df_cache,
                                          argparse.Namespace(**{**vars(args), "overwrite": True}),
                                          outdir)
            else:
