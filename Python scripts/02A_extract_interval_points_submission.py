...
def process_interval(row, flight_df_cache, args, outdir: Path):
    ...
    if flight_id not in flight_df_cache:
        parquet_path = Path(args.root) / "flights_rank" / f"{flight_id}.parquet"
...
def main():
    ...
    ap.add_argument("--root", default="prc-2025-datasets",
                    help="Dataset root containing flights_rank/")
    ap.add_argument("--features", default="data/processed/submission_intervals.csv",
                    help="CSV with idx,flight_id,start,end,... (submission set)")
    ap.add_argument("--outdir", default="data/processed/Intervals_submission",
