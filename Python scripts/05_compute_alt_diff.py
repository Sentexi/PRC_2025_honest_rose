...
"""
Schnelle Berechnung von alt_diff & vs_mean_fpm mit parallelem I/O.

- Liest data/processed/features_intervals.csv
- Für jedes idx: data/processed/Intervals/[idx]_flight_data.csv
  -> streamt nur 'timestamp','altitude' (keine Datumsparse, keine Sortierung)
  -> nimmt erste und letzte NICHT-NA-Höhe in Dateireihenfolge
- Schreibt data/processed/features_intervals_v1.csv
...
# --- NEU: mittlere Flughöhe streamend ermitteln -----------------------------
...
def process_one(idx_val, points_dir):
    ...
    if not exists:
        return (0, np.nan)
    return (1, alt_diff_stream(points_path))

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--intervals-csv", default="data/processed/features_intervals.csv",
                        help="Pfad zu data/processed/features_intervals.csv")
    parser.add_argument("--points-dir", default="data/processed/Intervals",
                        help="Ordner mit [idx]_flight_data.csv")
    parser.add_argument("--out-csv", default="data/processed/features_intervals_v1.csv",
                        help="Zieldatei")
    ...
    if "idx" not in df.columns:
        print("ERROR: Spalte 'idx' fehlt in features_intervals.csv", file=sys.stderr)
        sys.exit(1)
    ...
    worker_fn = partial(process_one, points_dir=args.points_dir)
    ...
    df_out["alt_mean_ft"]  = alt_means   # <- NEU
