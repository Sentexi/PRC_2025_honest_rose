...
def load_sources(root: Path):
    ...
    fl = pd.read_parquet(root / "flightlist_final.parquet", engine="pyarrow", columns=fl_cols)
    fl = fl.rename(columns={"destination_icao": "dest_icao"})

    fu = pd.read_parquet(root / "fuel_final_submission.parquet", engine="pyarrow")
...
def join_airports_to_flightlist(flightlist: pd.DataFrame, apt: pd.DataFrame) -> pd.DataFrame:
    ...
    fl["gc_distance_km"]   = haversine_km(fl["origin_lat"], fl["origin_lon"],
...
def build_interval_features(root: Path) -> pd.DataFrame:
    ...
    # ---- NEW: drop intervals with no flight-level data ----
    if "takeoff" in df.columns:
        missing = df["takeoff"].isna().sum()
        if missing:
            print(f"Dropping {missing:,} intervals with no matching flight in flightlist_final.")
            df = df[df["takeoff"].notna()].copy()
    ...
    if {"start","end"}.issubset(df.columns):
        ...
        for c in ["interval_min","t_since_takeoff_min","t_to_landing_min",
                  "pct_elapsed_mid","start_hour_utc","end_hour_utc"]:
            df[c] = np.nan
    ...
    float_cols = [c for c in num_cols
                  if np.issubdtype(df[c].dtype, np.floating) and c not in latlon_skip]
    for c in float_cols:
        s = df[c]
        mask = s.notna() & ((s % 1) != 0)
...
def main():
    ap = argparse.ArgumentParser(
        description="Build final submission intervals CSV (no fuel labels). Leaves fuel_kg_min empty; merges flightlist_final + fuel_final_submission + apt."
    )
    ap.add_argument("--root", default="prc-2025-datasets", help="Dataset root")
    ap.add_argument("--out",  default=r"data\processed\final_submission_intervals.csv", help="Output CSV path")
